...
## Context
Determine the batch to review using the FIRST matching method:
1. **User-specified:** If the user names a batch (e.g., `B007`), use that.
2. **Git branch:** Run `git branch --show-current`. If the branch matches `batch/BXXX`, that `BXXX` is the batch ID.
3. **CLAUDE.md fallback:** Read CLAUDE.md "Batch Queue" table and find the row with status `in-progress`.

Once you have the batch ID, read `docs/batches/{batch-id}-*.md` for the batch document.

## Description
Run by a FRESH agent after implementation and retro are complete. This system handles food commodity trade data — customer names and pricing are commercially sensitive, and uploaded documents (COAs, lab reports) are access-controlled.
...
### Injection
- SQL injection via unsanitised input in `better-sqlite3` queries — are all queries parameterised?
...
- Per security rules: restricted file categories return **404** (not 403) for unauthorised access
- other issues related to authentication & Authorization
...
- Any other potential Secure Data Leakage

### File Upload & Serving
- File paths constructed from user input without sanitisation + resolve + prefix check
...
- Any other security issues related to File Upload & Serving
...
### Input Validation
- User-controlled values used in file paths, SQL, or redirects without sanitisation
...
- [ ] All user input validated before use in file paths (sanitise + resolve + prefix)
- [ ] No customer names, pricing, or sensitive ERP fields in any output
- [ ] Unauthorised access returns 404 (not 403) for file/document routes
