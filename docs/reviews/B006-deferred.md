...
|---|--------|----------|------|---------|----------------|-------------|--------|
| D1 | integration | Minor | lib/agent-tools.ts | `apply_sync` doesn't pre-validate file existence like `run_sync_diff` does — generic "Sync failed" gives no actionable guidance | Requirements explicitly designed this way (EC1-EC3); system prompt rule 14 enforces workflow order ensuring files exist | B007 or TBD | OPEN |
