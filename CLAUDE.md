...
- **Old-format files** — files uploaded before this naming convention used `{unix-timestamp}-{original-filename}`. These can be batch-renamed via the Admin Tools page (`/admin/tools`) or `npm run rename-uploads`.
...
- `app/api/rename-uploads/route.ts` — GET preview / POST execute batch rename of old-format files (reviewer auth, runs inside Railway service)
...
- `app/api/rename-uploads/route.ts` — GET preview / POST execute file rename (reviewer auth, runs inside Railway container)
- `app/admin/tools/` — Admin tools page with COA backfill and file rename UI (layout, page, BackfillClient)
- `scripts/backfill-coa.ts` — Standalone CLI backfill script (for local use; production uses API endpoint)
- `scripts/rename-uploads.ts` — Standalone CLI rename script (for local use; production uses API endpoint)
...
- **Document matching:** Upload a COA, spec sheet, label, or product photo via drag-and-drop or file picker → Claude reads the document, extracts lot numbers / contract numbers / product names / production date, proposes matches against inventory, and uploads after explicit user confirmation. The production date is passed as `documentDate` so filenames reflect the actual document date rather than today.
...
- Document uploads in `public/uploads/{product-id}/lots/{lot-number}/` and `public/uploads/{product-id}/contracts/{base-contract}/`. Filenames use the descriptive convention `YYYY-MM-DD. {Product} - {Type} - {Identifier}.ext` — generated by `generateDocFilename()` in `lib/documents.ts`
...
| `npm run rename-uploads` | Rename old-format uploaded files (unix-timestamp prefix) to descriptive `YYYY-MM-DD. {Product} - {Type} - {Identifier}.ext` names. Updates the `documents` table. Safe to run multiple times — skips already-renamed files. For production: use `/admin/tools` → Rename Uploaded Files (runs inside the Railway service where the volume is mounted). |
